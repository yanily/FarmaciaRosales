...
BEGIN
	declare cur_empleados cursor for
		select * from SUCURSAL1.dbo.EMP_ING_ACT_ELI;

	declare cur_empleadosTelefono cursor for
		select * from SUCURSAL1.dbo.EMP_TEL_ING_ACT_ELI;

	declare cur_empleadosCorreos cursor for
		select * from SUCURSAL1.dbo.EMP_CORREO_ING_ACT_ELI;

	declare cur_facturas cursor for
		select * from SUCURSAL1.dbo.FACT_ING_ACT_ELI;

	declare cur_factDetalles cursor for
		select * from SUCURSAL1.dbo.DET_FACT_ING_ACT_ELI;
	...
	begin
		if @tipo = 'I'
		begin
			select @nombre = nombre, @direccion = direccion, @salario = salario, @rol = rol, @idFarmacia = idFarmacia from SUCURSAL1.dbo.EMPLEADOS
		...
		begin
			select @idFarmacia = idFarmacia from SUCURSAL1.dbo.EMPLEADOS where idEmpleado = @idEmpleado;
	...
	while @@fetch_status = 0
	begin
		select @idFarmacia = idFarmacia from SUCURSAL1.dbo.EMPLEADOS_TELEFONOS where idEmpleado = @idEmpleado and telefono = @telefono;
		if @tipo = 'I'
		begin
	...
	while @@fetch_status = 0
	begin
		select @idFactura = idFarmacia from SUCURSAL1.dbo.EMPLEADOS_CORREOS where idEmpleado = @idEmpleado and correo = @correo;
		if @tipo = 'I'
		begin
	...
	begin
		...
		begin
			...
			@descuento = descuento, @IVA = IVA, @IV = IV, @correo = correo from SUCURSAL1.dbo.FACTURAS where idFactura = @idFactura;
		...
		begin
			select @fecha = fecha, @montoTotal = montoTotal, @idEmpleado = idEmpleado, @idFarmacia = idFarmacia, @tipoPago = tipoPago, 
			@descuento = descuento, @IVA = IVA, @IV = IV from SUCURSAL1.dbo.FACTURAS where idFactura = @idFactura;
		...
		begin
			select @idFarmacia = idFarmacia from SUCURSAL1.dbo.FACTURAS where idFactura = @idFactura;
	...
	begin
		if @tipo = 'I'
		begin
			select @costo = costo, @cantidad = cantidad, @subtotal = subtotal, @idFarmacia = idFarmacia from SUCURSAL1.dbo.DETALLEFACTURA
		...
		begin
			select @cantidad = cantidad, @subtotal = subtotal, @idFarmacia = idFarmacia from SUCURSAL1.dbo.DETALLEFACTURA
			where idFactura = @idFactura and idProducto = @idProducto;

			if not exists (select cantidad from SUCURSAL1.dbo.DETALLEFACTURA where idFactura = @idFactura and idProducto = @idProducto 
		...
		begin
			select @idFarmacia = idFarmacia from SUCURSAL1.dbo.DETALLEFACTURA where idFactura = @idFactura and idProducto = @idProducto;
