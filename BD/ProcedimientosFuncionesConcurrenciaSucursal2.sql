...
BEGIN
	BEGIN TRY
		...
		SELECT @existencias = existencias FROM SUCURSAL2.dbo.INVENTARIO WITH (HOLDLOCK)  WHERE idProducto = @idProducto;
		...
		begin
			...
			UPDATE SUCURSAL2.dbo.INVENTARIO SET existencias = @existencias WHERE idProducto = @idProducto;
